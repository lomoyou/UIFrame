...
#define SDL_EVENT_USER              (SDL_USEREVENT +1)

#define LCD_UPDATE_TEXT             0x01
#define LCD_UPDATE_IMAGE            0x02
#define LCD_UPDATE_BUTTON           0x03
#define LCD_UPDATE_QRCODE           0x04
#define LCD_UPDATE_VIDEO            0x05
#define LCD_UPDATE_WINDOW           0x06
#define LCD_UPDATE_CONTROLLER       0x07
#define LCD_UPDATE_RAISE            0x08
/* Private types -------------------------------------------------------------*/
...
static void handleUserEvent( SDL_UserEvent * user)
{
    if (user == NULL) return;

    if (user->code == LCD_UPDATE_TEXT)
    {
        LUpdateText_t *uText = (LUpdateText_t *)user->data1;
        pthread_mutex_lock(&uText->win->lock);
        UpdateLcdText(uText->text, uText->content, uText->opt);
        pthread_mutex_unlock(&uText->win->lock);

        free(uText);
    }

    if (user->code == LCD_UPDATE_IMAGE)
    {
        LUpdateImage_t *uImg = (LUpdateImage_t *)user->data1;
        pthread_mutex_lock(&uImg->win->lock);
        UpdateLcdImage(uImg->image, uImg->file);
        pthread_mutex_unlock(&uImg->win->lock);

        free(uImg);
    }

    if (user->code == LCD_UPDATE_BUTTON)
    {
        LUpdateButton_t *uBtn = (LUpdateButton_t *)user->data1;
        pthread_mutex_lock(&uBtn->win->lock);
        UpdateLcdButton(uBtn->btn, uBtn->icon);
        pthread_mutex_unlock(&uBtn->win->lock);
        free(uBtn);
    }

    if (user->code == LCD_UPDATE_WINDOW)
    {
        LcdWindow_t * win = (LcdWindow_t *)user->data1;
        pthread_mutex_lock(&win->lock);
        LcdWindowPresent(win->win, win->page, user->windowID);
        win->pgupdate = 0;
        pthread_mutex_unlock(&win->lock);
    }


    if(user->code == LCD_UPDATE_CONTROLLER)
    {
        LUpdateWindow_t * uWin = (LUpdateWindow_t *)user->data1;

        pthread_mutex_lock(&uWin->win->lock);
        if (uWin->show) SDL_ShowWindow(uWin->win->window);
        else SDL_HideWindow(uWin->win->window);
        pthread_mutex_unlock(&uWin->win->lock);

        free(uWin);
    }

    if(user->code == LCD_UPDATE_RAISE)
    {
        LcdWindow_t *win = (LcdWindow_t *)user->data1;
        pthread_mutex_lock(&win->lock);
...
int LCD_DisplayText         ( LcdWindow_t *win, int page, int id, char * val )
{
    ...
    while(o){
        ...
        if (o && o->page == page && o->type == OBJ_TYPE_TEXT && o->id == id) {
            ...
            if (txt != NULL) {
#if 1
                LUpdateText_t *uText = malloc(sizeof(LUpdateText_t));
                memset(uText, 0, sizeof(LUpdateText_t));

                uText->win = win;
                uText->text = txt;
                uText->opt = 1;
                strcpy(uText->content, val);
                uev.code = LCD_UPDATE_TEXT;
                uev.data1 = (void *)uText;
                uev.type = SDL_EVENT_USER;
...
int LCD_DisplayImage        ( LcdWindow_t *win, int page,  int id, char * file )
{
    ...
    while(o)
    {
        ...
        if( o && o->page == page && o->type == OBJ_TYPE_IMAGE && o->id == id )
        {
            ...
            if( img != NULL ){
#if 1
                LUpdateImage_t *uImage = malloc(sizeof(LUpdateImage_t));
                memset(uImage, 0, sizeof(LUpdateImage_t));
                uImage->win = win;
                uImage->image = img;
                strcpy(uImage->file, file);

                uev.type = SDL_EVENT_USER;
                uev.code = LCD_UPDATE_IMAGE;
                uev.data1 = (void *)uImage;
...
#if 1
                LUpdateButton_t *uButton = malloc(sizeof(LUpdateButton_t));
                memset(uButton, 0, sizeof(LUpdateButton_t));
                uButton->btn = btn;
                uButton->win = win;
                if(file != NULL) strcpy(uButton->icon, file);
                uev.type = SDL_EVENT_USER;
                uev.code = LCD_UPDATE_BUTTON;
                uev.data1 = (void *)uButton;
...
int32_t LCD_WindowControl            ( LcdWindow_t *win, int show)
{
    ...
    LUpdateWindow_t *uWin = malloc(sizeof(LUpdateWindow_t));
    uWin->win = win;
    uWin->show = show;

    uev.type = SDL_EVENT_USER;
    uev.code = LCD_UPDATE_CONTROLLER;
    uev.data1 = (void *)uWin;
    uev.data2 = NULL;
